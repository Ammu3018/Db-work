...
const testPostEmployees = async function (req, res) {
  let err;
  let body = req.body;
  [err, employee] = await to(Test.create(body));
  if (err) return ReE(res, err, 422);
  return (ReS(res, { employee }));
}
module.exports.testPostEmployees = testPostEmployees;

const testGetEmployees = async function (req, res) {
  let err;
  [err, employee] = await to(Test.findAll());
  if (err) return ReE(res, err, 422);
  return (ReS(res, { employee }));
}
module.exports.testGetEmployees = testGetEmployees;
